...
var _ contract.Converter = (*tiffToJpegConverter)(nil)

// tiffToJpegConverter TIFF -> JPEG 文件转换器
...
func (s *tiffToJpegConverter) From() contract.Concept {
	return contract.TIFF()
}

func (s *tiffToJpegConverter) To() contract.Concept {
	return contract.JPEG()
}

func (s *tiffToJpegConverter) Params() []contract.ConverterParam {
...
func (s *tiffToJpegConverter) Convert(ctx context.Context, in []byte, params map[string]string) ([]byte, error) {
	...
	// Decode TIFF
	img, err := tiff.Decode(bytes.NewReader(in))
	if err != nil {
		return nil, exception.Wrapf(exception.Join(exception.ErrConvertFailed, err), "tiff decode failed")
	}
