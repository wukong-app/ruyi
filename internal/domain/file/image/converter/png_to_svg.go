...
var _ contract.Converter = (*pngToSvgConverter)(nil)

// pngToSvgConverter PNG -> SVG 文件转换器 (嵌入式)
type pngToSvgConverter struct {
...
func (s *pngToSvgConverter) From() contract.Concept {
	return contract.PNG()
}

func (s *pngToSvgConverter) To() contract.Concept {
	return contract.SVG()
}

func (s *pngToSvgConverter) Params() []contract.ConverterParam {
...
func (s *pngToSvgConverter) Convert(ctx context.Context, in []byte, params map[string]string) (out []byte, err error) {
	// 1. check params
	...
	if err != nil {
		return nil, exception.Wrapf(err, "invalid png image")
	}
	...
	// viewBox 保持原图尺寸，width/height 使用目标尺寸，这样浏览器会自动缩放
