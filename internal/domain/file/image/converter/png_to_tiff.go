...
var _ contract.Converter = (*pngToTiffConverter)(nil)

// pngToTiffConverter PNG -> TIFF 文件转换器
...
func (s *pngToTiffConverter) From() contract.Concept {
	return contract.PNG()
}

func (s *pngToTiffConverter) To() contract.Concept {
	return contract.TIFF()
}

func (s *pngToTiffConverter) Params() []contract.ConverterParam {
...
func (s *pngToTiffConverter) Convert(ctx context.Context, in []byte, params map[string]string) ([]byte, error) {
	...
	if err = tiff.Encode(&buf, img, nil); err != nil {
		return nil, exception.Wrapf(exception.Join(exception.ErrConvertFailed, err), "tiff encode failed")
	}
