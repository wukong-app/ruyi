...
var _ contract.Converter = (*webpToJpegConverter)(nil)

// webpToJpegConverter WEBP -> JPEG 文件转换器
...
func (s *webpToJpegConverter) From() contract.Concept {
	return contract.WEBP()
}

func (s *webpToJpegConverter) To() contract.Concept {
	return contract.JPEG()
}

func (s *webpToJpegConverter) Params() []contract.ConverterParam {
...
func (s *webpToJpegConverter) Convert(ctx context.Context, in []byte, params map[string]string) ([]byte, error) {
	...
	if err != nil {
		return nil, exception.Wrapf(exception.Join(exception.ErrConvertFailed, err), "webp decode failed")
	}
