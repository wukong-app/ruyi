...
var _ contract.Converter = (*pngToGifConverter)(nil)

// pngToGifConverter PNG -> GIF 文件转换器
...
func (s *pngToGifConverter) From() contract.Concept {
	return contract.PNG()
}

func (s *pngToGifConverter) To() contract.Concept {
	return contract.GIF()
}

func (s *pngToGifConverter) Params() []contract.ConverterParam {
...
func (s *pngToGifConverter) Convert(ctx context.Context, in []byte, params map[string]string) ([]byte, error) {
	...
	// nil options means default options (256 colors)
	if err = gif.Encode(&buf, img, nil); err != nil {
		return nil, exception.Wrapf(exception.Join(exception.ErrConvertFailed, err), "gif encode failed")
	}
