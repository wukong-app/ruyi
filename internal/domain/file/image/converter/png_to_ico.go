...
var _ contract.Converter = (*pngToIcoConverter)(nil)

// pngToIcoConverter PNG -> ICO 文件转换器
type pngToIcoConverter struct {
...
func NewPNGToICOConverter() contract.Converter {
	...
	return &pngToIcoConverter{
		params: params,
	}
}

func (s *pngToIcoConverter) From() contract.Concept {
	return contract.PNG()
}

func (s *pngToIcoConverter) To() contract.Concept {
	return contract.ICO()
}

func (s *pngToIcoConverter) Params() []contract.ConverterParam {
...
func (s *pngToIcoConverter) Convert(ctx context.Context, in []byte, params map[string]string) ([]byte, error) {
	...
	if err = ico.Encode(&buf, img); err != nil {
		return nil, exception.Wrapf(exception.Join(exception.ErrConvertFailed, err), "ico encode failed")
	}
