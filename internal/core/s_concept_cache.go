...
type conceptCache struct {
	...
	nameOrAliasesMap map[string]Concept
...
func newConceptCache() *conceptCache {
	return &conceptCache{
		nameOrAliasesMap: make(map[string]Concept),
...
func (s *conceptCache) putInNameOrAliasesMap(concept Concept) {
	if s.nameOrAliasesMap == nil {
		s.nameOrAliasesMap = make(map[string]Concept)
	}
...
func (s *conceptCache) getFromByNameOrAliasesMap(name string) (concept Concept, exist bool) {
